...
-- Refresh all dynamic menu entries from current mpv properties.
-- This is also used immediately before opening the menu so track/chapter/playlist
-- submenus cannot remain in their pre-playback empty state.
local function refresh_dynamic_menus()
...
mp.add_key_binding('MBTN_RIGHT', 'show', function()
    -- Rebuild dynamic menus immediately before opening. This fixes menus such as
    -- 轨道/章节/播放列表 that may be empty during idle and become populated after a file loads.
    refresh_dynamic_menus()
...
-- Refresh dynamic menus after a file becomes available. The deferred calls handle
-- players where track/chapter properties are populated slightly after file-loaded.
local function schedule_dynamic_refreshes()
    mp.add_timeout(0.05, function() pcall(refresh_dynamic_menus) end)
    mp.add_timeout(0.20, function() pcall(refresh_dynamic_menus) end)
end
mp.register_event('file-loaded', schedule_dynamic_refreshes)
mp.register_event('start-file', schedule_dynamic_refreshes)
